...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        if ($adminJobObj->updateJob($jobId, $formData)) {
            $_SESSION['message'] = "Job updated successfully!";
            $_SESSION['message_type'] = "success";
            header("Location: ../edit-jobs.php");
            exit;
        } else {
            $errors[] = "Error updating job";
        }
    }
} else {
    // For GET request, set formData from database
    $formData = $job;
...
$breadcrumbs = [
    ...
    ['title' => 'Edit Job', 'link' => '', 'active' => true]
];
                                ...
                                <select class="form-select" id="category_id" name="category_id" required>
                                    ...
                                    <?php foreach ($categories as $category): ?>
                                        <option value="<?php echo $category['idcategory']; ?>"
                                            <?php echo ($formData['category_idcategory'] == $category['idcategory']) ? 'selected' : ''; ?>>
